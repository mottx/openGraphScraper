...
var setOptionsAndReturnOpenGraphResults = function (options, callback) {
  ...
  if (validate.returnInputUrl) {
    ...
    return requestAndResultsFormatter(options, function (error, results, response) {
      ...
      return callback(true, { error: error, success: false, requestUrl: options.url, errorDetails: error, response: response }, response);
    });
  }
...
var requestAndResultsFormatter = function (options, callback) {
  ...
  const req = request(options, function (error, response, body) {
    ...
    if (options.maxContentLength && res.headers['content-length'] > options.maxContentLength) {
      req.abort();
      return callback('Request is aborted due to content length exceeds limit ' + options.maxContentLength + ' bytes');
    } else if (options.responseFilter && !options.responseFilter(res)) {
      req.abort();
      return callback('Request is aborted by response filter');
    }
  });
};
