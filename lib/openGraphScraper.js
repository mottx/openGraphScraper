    ...
    requestAndResultsFormatter(options, function (error, results, response) {
      if (results) {
        ...
        if (error && (error.code === 'ENOTFOUND' || error.code === 'EHOSTUNREACH')) {
          ...
          return callback(true, { error: error, success: false, requestUrl: options.url, errorDetails: error }, response);
        }
      }
    });
  } else {
...
const requestAndResultsFormatter = function (options, callback) {
  ...
  let req = request(options, function (error, response, body) {
    ...
    } else if (options.contentTypeFilter && !options.contentTypeFilter(res)) {
      req.abort();
      return callback('Request is aborted by content type filter');
    }
  });
};
